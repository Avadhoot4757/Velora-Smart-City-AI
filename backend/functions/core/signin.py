...
API_KEY = os.getenv('FIREBASE_API_KEY', 'YOUR_API_KEY')

@https_fn.on_request(region='us-central1')
def signin(req: https_fn.Request) -> https_fn.Response:
    headers = {
    ...
    try:
        ...
        if provider == 'email':
            ...
            result = firebase_auth_api('accounts:signInWithPassword', data, API_KEY)
            res.status = 200
            res.set_json({'message': 'User signed in', 'idToken': result['idToken']})
            ...
            res.status = 200
            res.set_json({'message': 'User signed in with Google', 'idToken': result['idToken']})
